...
- [ ] Add a collapsible right sidebar to the UI (`cmd/varlet-server/web/index.html`).
- [ ] Bind Cytoscape graph node click events to open the sidebar.
- [ ] Extract detailed status information (from `NamespaceStatusInfo`) for the selected node and display:
...
- [ ] Show empty/placeholder UI states if there are no causal UUIDs or no previous actuations.
